...
abstract class TestCase extends BaseTestCase
{
    ...
    protected function seedFullStudent(array $overrides = []): int
    {
        ...
        $defaults = [
            ...
            'enrolment_approval_status'=> null,
            'login_enabled'            => 0,
            'created_by'               => 1,
        ...
        $this->pdo->prepare(
            "INSERT INTO students
             (first_name, last_name, dob, mobile, gender, department_id, programme_level,
              ...
              enrolment_approval_status, login_enabled, created_by, upload_batch_id, created_at)
             VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
        )->execute([
            ...
            $row['enrolment_approval_status'], $row['login_enabled'], $row['created_by'],
            $row['upload_batch_id'], $row['created_at'],
        ]);
        return (int)$this->pdo->lastInsertId();
    }
}
